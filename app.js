...
function cacheElements() {
  [
    ...
    'goToTypesBtn', 'addTypeBtn', 'typesList', 'calculatorItems', 'startPriceSummary',
    'calculateBtn', 'resultBox', 'resetCalculatorBtn', 'typeModal', 'typeModalTitle',
    'typeForm', 'typeId', 'typeName', 'typeWorkSeconds', 'typeWalkSeconds', 'typePrice',
    ...
    'customerSearch', 'addCustomerBtn', 'customerPanel', 'closeCustomerPanelBtn',
...
async function normalizeTypeOrder() {
  ...
  ordered.forEach((item, index) => {
    const newOrder = item.isStartPrice ? 0 : index;
...
function sortByOrderThenName(a, b) {
  if ((a.isStartPrice ? 0 : 1) !== (b.isStartPrice ? 0 : 1)) {
    return (a.isStartPrice ? 0 : 1) - (b.isStartPrice ? 0 : 1);
  }
...
function renderTypes() {
  ...
  windowTypes.forEach((type, index) => {
    ...
    const moveUpDisabled = type.isStartPrice || (index <= 1 && !type.isStartPrice);
    const moveDownDisabled = type.isStartPrice || index === windowTypes.length - 1;

    card.innerHTML = `
      <div class="type-card-header">
        <div>
          <strong>${escapeHtml(type.name)}</strong>
          <p>${type.isStartPrice ? 'Fast post der altid lægges til' : 'Vinduestype'}</p>
        </div>
        <div class="type-actions">
          ...
          ${type.isStartPrice ? '' : `<button class="mini-btn danger" data-action="delete" data-id="${type.id}">Slet</button>`}
        </div>
      </div>
      <div class="type-meta">
        <span class="meta-pill ${type.isStartPrice ? 'fixed' : ''}">Pris: ${formatCurrency(type.price)}</span>
        <span class="meta-pill">Arbejde: ${formatSeconds(type.workSeconds)}</span>
        <span class="meta-pill">Gå tid: ${formatSeconds(type.walkSeconds)}</span>
      </div>
    `;
...
async function handleTypeSubmit(event) {
  ...
  const payload = {
    ...
    name: isStartPrice ? START_PRICE_NAME : els.typeName.value.trim(),
...
function getRegularTypes() {
  return windowTypes.filter((item) => !item.isStartPrice).sort(sortByOrderThenName);
}
...
function calculatePrice() {
  ...
  const regularTypes = getRegularTypes();
  const rows = [];
  let totalPrice = toMoney(startPrice?.price || 0);
  let totalWorkSeconds = toNumber(startPrice?.workSeconds || 0);
  let totalWalkSeconds = toNumber(startPrice?.walkSeconds || 0);
  ...
  regularTypes.forEach((type) => {
    ...
    const lineWalkSeconds = quantity * toNumber(type.walkSeconds);

    totalPrice += linePrice;
    totalWorkSeconds += lineWorkSeconds;
    totalWalkSeconds += lineWalkSeconds;
      ...
      breakdown.push(`
        <div class="result-line">
          <span>
            ...
            <small>${item.quantity} stk × ${formatCurrency(item.price)}</small>
          </span>
  ...
  els.resultBox.innerHTML = `
    <div class="result-panel">
      <div class="result-summary">
        <span class="meta-pill fixed">Total pris: ${formatCurrency(result.totalPrice)}</span>
      ...
      <div class="result-breakdown">
        ${breakdown.join('') || '<div class="empty-state">Ingen antal angivet endnu. Kun Start Pris er med, hvis den har en værdi.</div>'}
      </div>
...
function openCustomerModal({ fromCalculation }) {
  ...
  if (fromCalculation && latestCalculation) {
    ...
    els.customerQuoteSummary.innerHTML = `
      ...
      <p>Pris: ${formatCurrency(latestCalculation.totalPrice)} · Samlet tid: ${formatSeconds(latestCalculation.totalSeconds)} · Typer: ${activeLines}</p>
    `;
  } else {
...
function renderQuoteEditor(quote) {
  ...
  totals.innerHTML = `
    <span class="meta-pill fixed">Total pris: ${formatCurrency(quote.totalPrice || 0)}</span>
...
function updateQuoteSummaryPreview() {
  ...
  if (totals) {
    totals.innerHTML = `
      <span class="meta-pill fixed">Total pris: ${formatCurrency(editedQuote.totalPrice || 0)}</span>
...
function collectQuoteFromPanel() {
  ...
  const totalPrice = toMoney(startPrice.price + items.reduce((sum, item) => sum + toMoney(item.linePrice), 0));
  const totalSeconds = startCombined + items.reduce((sum, item) => sum + toNumber(item.lineWorkSeconds), 0);
