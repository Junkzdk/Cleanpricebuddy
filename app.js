...
function bindUI() {
  ...
  els.interiorEnabled.addEventListener('change', () => { if (latestCalculation) calculatePrice(); });
...
async function ensureDefaultData() {
  ...
  const interior = types.find((item) => item.isInterior);
...
async function normalizeTypeOrder() {
  ...
  ordered.forEach((item) => {
    const newOrder = item.isStartPrice ? 0 : item.isInterior ? 1 : regularOrder++;
...
function sortByOrderThenName(a, b) {
  if (getTypeSortRank(a) !== getTypeSortRank(b)) {
    return getTypeSortRank(a) - getTypeSortRank(b);
  }
...
function getTypeSortRank(type) {
  if (type?.isStartPrice) return 0;
  if (type?.isInterior) return 1;
  return 2;
}

function isFixedType(type) {
  return Boolean(type?.isStartPrice || type?.isInterior);
}
...
function renderTypes() {
  ...
  windowTypes.forEach((type, index) => {
    ...
    const moveUpDisabled = isFixedType(type) || (index <= 2 && !isFixedType(type));
    const moveDownDisabled = isFixedType(type) || index === windowTypes.length - 1;
    const typeDescription = type.isStartPrice
      ? 'Fast post der altid lægges til'
      : type.isInterior
        ? 'Fast post for indvendig - aktiveres i udregneren'
        : 'Vinduestype';
    ...
    const metaPills = [
      ...
      `<span class="meta-pill">Gå tid: ${formatSeconds(type.walkSeconds)}${isFixedType(type) ? '' : ` / hver ${toNumber(type.walkEveryCount || 1)} stk`}</span>`
    ].join('');
...
function renderCalculator() {
  ...
  const interiorType = getInteriorType();
  ...
  els.interiorSummary.innerHTML = interiorType
    ? `
      <span class="meta-pill fixed">Startpris: ${formatCurrency(interiorType.price)}</span>
      <span class="meta-pill">Arbejde: ${formatSeconds(interiorType.workSeconds)}</span>
      <span class="meta-pill">Gå tid: ${formatSeconds(interiorType.walkSeconds)}</span>
    `
    : '<span class="meta-pill">Indvendig mangler</span>';
...
function openTypeModal(type = null) {
  ...
  const isFixed = Boolean(type?.isStartPrice || type?.isInterior);
  const walkEveryWrap = els.typeWalkEveryCount.closest('label');

  if (type?.isStartPrice) {
    els.typeName.value = START_PRICE_NAME;
    els.typeName.disabled = true;
  } else if (type?.isInterior) {
    els.typeName.value = INTERIOR_NAME;
    els.typeName.disabled = true;
  } else {
  ...
  if (walkEveryWrap) {
    walkEveryWrap.classList.toggle('hidden', isFixed);
  }
...
async function handleTypeSubmit(event) {
  ...
  const isInterior = existing?.isInterior || false;

  const payload = {
    id: id || crypto.randomUUID(),
    name: isStartPrice ? START_PRICE_NAME : isInterior ? INTERIOR_NAME : els.typeName.value.trim(),
    workSeconds: toNumber(els.typeWorkSeconds.value),
    walkSeconds: toNumber(els.typeWalkSeconds.value),
    walkEveryCount: isStartPrice || isInterior ? 1 : Math.min(12, Math.max(1, toNumber(els.typeWalkEveryCount.value || 1))),
...
function getInteriorType() {
  return windowTypes.find((item) => item.isInterior);
}

function getRegularTypes() {
  return windowTypes.filter((item) => !item.isStartPrice && !item.isInterior).sort(sortByOrderThenName);
}
...
function resetCalculatorInputs() {
  ...
  els.interiorEnabled.checked = false;
...
function calculatePrice() {
  ...
  let exteriorPrice = toMoney(startPrice?.price || 0);
  let exteriorWorkSeconds = toNumber(startPrice?.workSeconds || 0);
  let exteriorWalkSeconds = toNumber(startPrice?.walkSeconds || 0);
  ...
  regularTypes.forEach((type) => {
    ...
    exteriorPrice += linePrice;
    exteriorWorkSeconds += lineWorkSeconds;
    exteriorWalkSeconds += lineWalkSeconds;
  ...
  const interiorLinesPrice = rows.reduce((sum, item) => sum + toMoney(item.linePrice), 0);
  const interiorLinesWorkSeconds = rows.reduce((sum, item) => sum + toNumber(item.lineWorkSeconds), 0);
  const interiorLinesWalkSeconds = rows.reduce((sum, item) => sum + toNumber(item.lineWalkSeconds), 0);
  const interiorStartPrice = interiorEnabled ? toMoney(interiorType?.price || 0) : 0;
  const interiorStartWorkSeconds = interiorEnabled ? toNumber(interiorType?.workSeconds || 0) : 0;
  const interiorStartWalkSeconds = interiorEnabled ? toNumber(interiorType?.walkSeconds || 0) : 0;

  const totalPrice = toMoney(exteriorPrice + (interiorEnabled ? interiorLinesPrice + interiorStartPrice : 0));
  const totalWorkSeconds = exteriorWorkSeconds + (interiorEnabled ? interiorLinesWorkSeconds + interiorStartWorkSeconds : 0);
  const totalWalkSeconds = exteriorWalkSeconds + (interiorEnabled ? interiorLinesWalkSeconds + interiorStartWalkSeconds : 0);
  ...
  latestCalculation = {
    ...
    startPrice: startPrice
      ...
      : null,
    interiorEnabled,
    interiorStart: interiorType
      ? {
          name: interiorType.name,
          price: toMoney(interiorType.price),
          workSeconds: toNumber(interiorType.workSeconds),
          walkSeconds: toNumber(interiorType.walkSeconds)
        }
      : null,
    items: rows,
    exteriorTotalPrice: toMoney(exteriorPrice),
    exteriorWorkSeconds,
    exteriorWalkSeconds,
    exteriorTotalSeconds: exteriorWorkSeconds + exteriorWalkSeconds,
    interiorLinesPrice: toMoney(interiorLinesPrice),
    interiorLinesWorkSeconds,
    interiorLinesWalkSeconds,
    totalPrice,
    totalWorkSeconds,
    totalWalkSeconds,
...
function renderCalculationResult(result) {
  ...
  if (result.startPrice) {
    breakdown.push(`
      <div class="result-line">
        <span><strong>${escapeHtml(result.startPrice.name)}</strong><br><small>Fast tillæg</small></span>
      ...
      breakdown.push(`
        <div class="result-line">
          <span>
            ...
            <small>${item.quantity} stk × ${formatCurrency(item.price)} · gå ${item.walkTriggers || 0}× efter hver ${toNumber(item.walkEveryCount || 1)} stk</small>
          </span>
  ...
  if (result.interiorEnabled) {
    breakdown.push(`
      <div class="result-line">
        <span><strong>Indvendig - samme vinduer</strong><br><small>Ekstra indvendig pris for alle valgte linjer</small></span>
        <span>${formatCurrency(result.interiorLinesPrice || 0)}</span>
      </div>
    `);
    if (result.interiorStart && (result.interiorStart.price || result.interiorStart.workSeconds || result.interiorStart.walkSeconds)) {
      breakdown.push(`
        <div class="result-line">
          <span><strong>${escapeHtml(result.interiorStart.name)}</strong><br><small>Fast indvendigt tillæg</small></span>
          <span>${formatCurrency(result.interiorStart.price || 0)}</span>
        </div>
      `);
    }
  }
  ...
  els.resultBox.innerHTML = `
    <div class="result-panel">
      <div class="result-summary">
        <span class="meta-pill fixed">Udvendig pris: ${formatCurrency(result.exteriorTotalPrice || 0)}</span>
        <span class="meta-pill ${result.interiorEnabled ? 'fixed' : ''}">Udvendig + indvendig pris: ${result.interiorEnabled ? formatCurrency(result.totalPrice) : 'Ikke aktiv'}</span>
        <span class="meta-pill">Arbejdstid: ${formatSeconds(result.totalWorkSeconds)}</span>
        <span class="meta-pill">Gå tid: ${formatSeconds(result.totalWalkSeconds)}</span>
        <span class="meta-pill">Samlet tid: ${formatSeconds(result.totalSeconds)}</span>
      </div>
      <div class="result-breakdown">
        ${breakdown.join('') || '<div class="empty-state">Ingen antal angivet endnu. Kun faste poster er med, hvis de har en værdi.</div>'}
      </div>
...
function openCustomerModal({ fromCalculation }) {
  ...
  if (fromCalculation && latestCalculation) {
    ...
    els.customerQuoteSummary.innerHTML = `
      ...
      <p>Udvendig pris: ${formatCurrency(latestCalculation.exteriorTotalPrice || 0)} · Udvendig + indvendig: ${latestCalculation.interiorEnabled ? formatCurrency(latestCalculation.totalPrice) : 'Ikke aktiv'} · Samlet tid: ${formatSeconds(latestCalculation.totalSeconds)} · Typer: ${activeLines}</p>
    `;
  } else {
...
function renderQuoteEditor(quote) {
  ...
  totals.innerHTML = `
    ...
    <span class="meta-pill ${quote.interiorEnabled ? 'fixed' : ''}">Udvendig + indvendig: ${quote.interiorEnabled ? formatCurrency(quote.totalPrice || 0) : 'Ikke aktiv'}</span>
...
function updateQuoteSummaryPreview() {
  ...
  if (totals) {
    totals.innerHTML = `
      <span class="meta-pill fixed">Udvendig pris: ${formatCurrency(editedQuote.exteriorTotalPrice ?? editedQuote.totalPrice ?? 0)}</span>
      <span class="meta-pill ${editedQuote.interiorEnabled ? 'fixed' : ''}">Udvendig + indvendig: ${editedQuote.interiorEnabled ? formatCurrency(editedQuote.totalPrice || 0) : 'Ikke aktiv'}</span>
      <span class="meta-pill">Samlet tid: ${formatSeconds(editedQuote.totalSeconds || 0)}</span>
    `;
  }
}
...
function collectQuoteFromPanel() {
  ...
  const existingQuote = customer.quote || {};
  ...
  const exteriorItemsPrice = items.reduce((sum, item) => sum + toMoney(item.linePrice), 0);
  const exteriorItemsSeconds = items.reduce((sum, item) => sum + toNumber(item.lineWorkSeconds), 0);
  const exteriorTotalPrice = toMoney(startPrice.price + exteriorItemsPrice);
  const exteriorTotalSeconds = startCombined + exteriorItemsSeconds;

  const interiorEnabled = Boolean(existingQuote.interiorEnabled);
  const interiorStart = existingQuote.interiorStart || { name: INTERIOR_NAME, price: 0, workSeconds: 0, walkSeconds: 0 };
  const interiorStartSeconds = toNumber(interiorStart.workSeconds || 0) + toNumber(interiorStart.walkSeconds || 0);
  const totalPrice = toMoney(exteriorTotalPrice + (interiorEnabled ? exteriorItemsPrice + toMoney(interiorStart.price || 0) : 0));
  const totalSeconds = exteriorTotalSeconds + (interiorEnabled ? exteriorItemsSeconds + interiorStartSeconds : 0);
  ...
  return {
    ...
    items,
    exteriorTotalPrice,
    exteriorWorkSeconds: exteriorTotalSeconds,
    exteriorWalkSeconds: 0,
    exteriorTotalSeconds,
    interiorLinesPrice: interiorEnabled ? exteriorItemsPrice : 0,
    interiorLinesWorkSeconds: interiorEnabled ? exteriorItemsSeconds : 0,
    interiorLinesWalkSeconds: 0,
    totalPrice,
...
function loadSelectedCustomerIntoCalculator() {
  ...
  els.interiorEnabled.checked = Boolean(customer.quote.interiorEnabled);
